//Azmi Utku Sezgin 131044048 HW03
...
int main(){
	...
	if(singleOrMulti =='S'){
		...
		for(int i=0; i< gameCount; ++i){
			cout << "Object" << multiGame[i].getObjectNumber() << ": " << endl;
			cout << "Please enter Height from 4 to 50" << endl;
			properInput = 0;
			while(properInput == 0){
				cin >> input;
				number = multiGame[i].stringToInt(input);
				if( number >= MIN_HEIGHT && number <= MAX_HEIGHT ){
					...
					cerr << "Wrong number please enter a number from 4 to 50" << endl;		
					cin.clear();
				}
				if(properInput == 1){
					...
					if(number < MIN_WIDTH || number > MAX_WIDTH){
						cerr << "Board Width cannot be" << number << endl;
						properInput=0;
						cin.clear();
					}
					else{
			...
			//Resizing board
			multiGame[i].resizeBoard(maxHeight, maxWidth);
