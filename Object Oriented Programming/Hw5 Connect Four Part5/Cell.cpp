...
myNameSpace::Cell::Cell(int w, int h){
	if(w>= 0 && w <26)
		width = w;
	else{
...
void myNameSpace::Cell::setWidth(int w){
	if(w>= 0 && w <26)
		width = w;
	else{
		cerr << "Cell column cannot be " << w << endl;
...
void myNameSpace::Cell::setHeight(int w){
	if(w>=0)
		height = w;	
}

void myNameSpace::Cell::setStatus(string x){
...
bool myNameSpace::Cell::operator==(const Cell& c) const{
...
myNameSpace::Cell& myNameSpace::Cell::operator++(){
...
myNameSpace::Cell myNameSpace::Cell::operator++(int ignoreMe){
	Cell temp(*this);
